...
- 询问你选择 `/auto-run` 还是 `/sandbox` 模式，并测试命令是否能自动执行
- 通过 iMessage 向你发送引导设置问题（在手机上回答即可）
- 根据你的回答构建人格和记忆，开始监听和自动回复

启动过程中 Agent 会在终端里问你这些问题：

| 问题 | 回答示例 | 说明 |
|---|---|---|
| 选择哪个通讯渠道？ | *iMessage* | 目前仅支持 iMessage |
| 通讯对象的手机号或 Apple ID？ | *+8613800138000* | 你自己的手机号或 Apple ID |
| 选择 `/auto-run` 还是 `/sandbox`？ | 输入 `/auto-run` 或 `/sandbox` | auto-run 更方便，sandbox 更安全 |
| 命令是否自动执行了？ | *是* | Agent 会运行测试命令并询问你确认 |

随后 Agent 会通过 iMessage 问你这些问题（在手机上回答）：
